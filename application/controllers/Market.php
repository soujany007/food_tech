...
class Market extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('Market_manager');
        $this->mrktm = $this->Market_manager;
    }
    ...
    function index(){
        ...
        $vars['meta_keywords']      = '';
        $vars['heading']            = 'Market List';
        $vars['content_view']       = 'market';
        $this->load->view('list_layout',$vars);
    }

    function addEditMarket($market_id='', $customWidth='')
    {
        /*
        checkLoginSession();
        $user_session_data = getUserSession();        
        ...
        $vars['meta_keywords'] = '';
        $vars['heading'] = $head;
        $vars['content_view'] = 'add_edit_market';
        $this->load->view('list_layout',$vars);
        */
        checkLoginSession();
        $error='';
        ...
        if(!empty($user_session_data)){
            ...
            if(!empty($market_id)){
                $head = 'Edit';
                //$market_data =$this->rpm->getCompanyDataById($market_id);
            ...
            $vars['dataArr']= !empty($market_data) ? $market_data : $this->input->post();
            //echo "<pre>";print_r($vars['dataArr']);die();
            $data = $this->load->view('add_edit_market',$vars,true);
            $returnArr['data'] = $data;
