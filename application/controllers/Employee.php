...
class Employee extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('Employee_manager');
        $this->rpm = $this->Employee_manager;
    }
    ...
    function index(){
        ...
        $vars['meta_keywords']      = '';
        $vars['heading']            = 'Employee List';
        $vars['content_view']       = 'employee';
        $this->load->view('list_layout',$vars);
    }

    function addEditEmployee($employee_id='', $customWidth='')
    {
        /*
        checkLoginSession();
        $user_session_data = getUserSession();        
        ...
        $vars['meta_keywords'] = '';
        $vars['heading'] = $head;
        $vars['content_view'] = 'add_edit_employee';
        $this->load->view('list_layout',$vars);
        */
        checkLoginSession();
        $error='';
        ...
        if(!empty($user_session_data)){
            ...
            if(!empty($employee_id)){
                $head = 'Edit';
                //$employee_data =$this->rpm->getCompanyDataById($employee_id);
            ...
            $vars['dataArr']= !empty($employee_data) ? $employee_data : $this->input->post();
            //echo "<pre>";print_r($vars['dataArr']);die();
            $data = $this->load->view('add_edit_employee',$vars,true);
            $returnArr['data'] = $data;
