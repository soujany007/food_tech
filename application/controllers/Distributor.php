...
class Distributor extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('Distributor_manager');
        $this->dstbtrm = $this->Distributor_manager;
    }
    ...
    function index(){
        ...
        $vars['meta_keywords']      = '';
        $vars['heading']            = 'Distributor List';
        $vars['content_view']       = 'distributor';
        $this->load->view('list_layout',$vars);
    }
    
    function addEditDistributor($distributor_id='', $customWidth='')
    {
        /*
        checkLoginSession();
        $user_session_data = getUserSession();        
        ...
        $vars['meta_keywords'] = '';
        $vars['heading'] = $head;
        $vars['content_view'] = 'add_edit_distributor';
        $this->load->view('list_layout',$vars);
        */
        checkLoginSession();
        $error='';
        ...
        if(!empty($user_session_data)){
            ...
            if(!empty($distributor_id)){
                $head = 'Edit';
                //$distributor_data =$this->rpm->getCompanyDataById($distributor_id);
            ...
            $vars['dataArr']= !empty($distributor_data) ? $distributor_data : $this->input->post();
            //echo "<pre>";print_r($vars['dataArr']);die();
            $data = $this->load->view('add_edit_distributor',$vars,true);
            $returnArr['data'] = $data;
