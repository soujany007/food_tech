...
class Return_product extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model('Return_product_manager');
        $this->rpm = $this->Return_product_manager;
    }
    ...
    function index(){
        ...
        $vars['meta_keywords']      = '';
        $vars['heading']            = 'Return Product List';
        $vars['content_view']       = 'return_product';
        $this->load->view('list_layout',$vars);
    }

    function addEditReturnProduct($return_product_id='', $customWidth='')
    {   
        /*
        checkLoginSession();
        $user_session_data = getUserSession();        
        ...
        $vars['meta_keywords'] = '';
        $vars['heading'] = $head;
        $vars['content_view'] = 'add_edit_return_product';
        $this->load->view('list_layout',$vars);
        */
        ...
        if(!empty($user_session_data)){
            ...
            if(!empty($return_product_id)){
                $head = 'Edit';
                //$company_data =$this->rpm->getCompanyDataById($return_product_id);
            ...
            $vars['dataArr']= !empty($return_product_data) ? $return_product_data : $this->input->post();
