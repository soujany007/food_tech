...
class Employee_beat_instance extends CI_Controller {

    function __construct() {
        parent::__construct();
        $this->qc = $this->queryconstant;

        $this->load->model('Employee_beat_instance_manager');
        $this->ebim = $this->Employee_beat_instance_manager;
    }
    ...
    function index(){
        ...
        $vars['meta_keywords']      = '';
        $vars['heading']            = 'Employee Beat Instance List';
        $vars['content_view']       = 'employee_beat_instance';
        $this->load->view('list_layout',$vars);
    }

    function addEditEmployeeBeatInstance($employee_beat_instance_id='', $customWidth='')
    {
        /*
        checkLoginSession();
        $user_session_data = getUserSession();        
        ...
        $vars['meta_keywords'] = '';
        $vars['heading'] = $head;
        $vars['content_view'] = 'add_edit_employee_beat_instance';
        $this->load->view('list_layout',$vars);
        */
        checkLoginSession();
        $error='';
        ...
        $employee_beat_instance_data=array();
        if(!empty($user_session_data)){
            $head = 'Add';
            if(!empty($employee_beat_instance_id)){
                $head = 'Edit';
                //$company_data =$this->rpm->getCompanyDataById($employee_beat_instance_id);
            ...
            $vars['dataArr']= !empty($employee_beat_instance_data) ? $employee_beat_instance_data : $this->input->post();
            //echo "<pre>";print_r($vars['dataArr']);die();
            $data = $this->load->view('add_edit_employee_beat_instance',$vars,true);
            $returnArr['data'] = $data;
